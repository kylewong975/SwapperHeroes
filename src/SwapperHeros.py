...
#get_ipython().system(u'apt-get install git -y')
#get_ipython().system(u'apt-get install wget -y')
#get_ipython().system(u'git clone https://github.com/junyanz/pytorch-CycleGAN-and-pix2pix.git')
...
ndf = 64
use_lsgan = True
flambda = 10 if use_lsgan else 100
...
loss_G = loss_GA+loss_GB+flambda*loss_cyc
...
def rescale_image(instring):
    print(instring)
    img = cv2.imread(instring)
    print("img.shape", img.shape)
    img = cv2.resize(img, (256, 256))
    cv2.imwrite(os.path.splitext(instring)[0]+'.jpg',img)
...
#data = "edges2shoes"
data = "horse2zebra"
train_A = load_data('./CycleGAN/data/{}/trainA/*'.format(data))
train_B = load_data('./CycleGAN/data/{}/trainB/*'.format(data))
for i in train_A:
    rescale_image(i)
for i in train_B:
    rescale_image(i)
...
def showX(X, rows=1):
    ...
    cv2.imwrite("/home/ubuntu/prog_gans_pytorch_inference/test.png",int_X)
...
while epoch < niter: 
    ...
    if gen_iterations%display_iters==0:
        ...
        showG(A,B)        
        errCyc_sum = errGA_sum = errGB_sum = errDA_sum = errDB_sum = 0
        netGA.save('./CycleGAN/data/{}.netGA'.format(gen_iterations)) 
        netGB.save('./CycleGAN/data/{}.netGB'.format(gen_iterations))
        netDA.save('./CycleGAN/data/{}.netDA'.format(gen_iterations))
        netDB.save('./CycleGAN/data/{}.netDB'.format(gen_iterations))
